...
server <- function(input, output, session) {
  ...
  assessment_record <- reactive({
    ...
    data.frame(
      field = c(
        ...
        # READUS-PV (Fusaroli et al., Drug Safety 2024) items 4-8: study
        # population, case definition, comparator, and case-by-case review must
        # be stated rather than left implicit. These were undisclosed before.
        "Study population", "Case definition", "Comparator (non-cases)",
      ...
      value = c(
        ...
        prov$query_window %||% "",
        nrow(df),
        paste("All FAERS individual case safety reports received in the query window.",
              "No restriction by age, sex, reporter type, country, or seriousness."),
        paste0("Reports listing ", toupper(trimws(input$live_drug)),
               " in ANY role. drugcharacterization is not filtered, so suspect,",
               " interacting and concomitant mentions are all counted; this",
               " inflates counts for widely co-prescribed drugs. The drug is",
               " matched across patient.drug.medicinalproduct,",
               " openfda.brand_name and openfda.generic_name. Event is an exact",
               " match on patient.reaction.reactionmeddrapt (MedDRA Preferred",
               " Term), so narrower and broader terms are not included."),
        paste("All other drug-event reports in FAERS within the same window",
              "(the full database serves as the non-case comparator; no",
              "restricted or active comparator is applied)."),
        paste("Not performed. Disproportionality analysis only \u2014 no individual",
              "case review, no causality assessment. A signal is a hypothesis",
              "for assessment, not a validated association."),
        ...
        R.version.string,
        paste("Disclosures follow the READUS-PV reporting guideline",
              "(Fusaroli et al., Drug Safety 2024). READUS-PV is a reporting",
              "standard for disproportionality analyses, not a regulatory",
              "requirement; items relating to manuscript structure do not apply",
              "to a per-query record."),
  ...
  # -- Methods: negative control arm (specificity) ----------------------------
  # negative_controls is NULL until scripts/02_signal_detection.R has run, and
  # R/50_ui.R is sourced at startup, so both outputs must render a placeholder
  ...
  output$negctl_summary <- renderUI({
    if (is.null(negative_controls)) {
      return(div(class = "alert alert-secondary py-2 px-3 mb-0",
                 style = "font-size:0.85rem;",
                 "Negative control results are generated by the quarterly pipeline ",
                 "and have not been computed for this build yet."))
    }
  ...
  output$negctl_table <- DT::renderDT({
    if (is.null(negative_controls)) {
      return(DT::datatable(
